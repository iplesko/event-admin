...
import EventForm from "../AddEventForm/EventForm";
...
export default (): React.ReactElement => {
  ...
  return (
      <>
        <EventForm heading="Add event" buttonText="Add" onConfirm={addEvent} visible={addFormVisible}
                   toggleVisibility={toggleAddFormVisible}/>
        <EventForm heading="Edit event" buttonText="Save" onConfirm={updateEvent} visible={editEvent !== null}
                   toggleVisibility={toggleEditFormVisible} displayedEvent={editEvent}/>
                ...
                <Col sm={4} key={e.id}>
                  <Card className={past && 'past-event'}>
                    <CardBody>
                      ...
                      <CardSubtitle className="text-muted">{formatDate(e.from)} - {formatDate(e.to)}</CardSubtitle>
